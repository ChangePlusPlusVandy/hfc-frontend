...
export const WorkshopSingle = () => {
    ...
    const deleteWorkshop = () =>{
        ...
        fetch("http://localhost:3000/workshops?_id=" + workshopID, requestOptions);  
    ...
    const updateWorkshop=()=>{
        ...
        const requestOptions = {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({title: updateWorkTitle, description: updateWorkDesc, archived: updateWorkStatus, hosts: updateWorkHosts.map((item)=>{return item.value}), date: updateDate})
        };
        console.log(requestOptions)
        fetch("http://localhost:3000/workshops?_id=" + workshopID, requestOptions).then(
            (response)=>{
            ...
            .then((data) => {
                ...
                .then((data2) => {
                    let tempOptions = data2
                        ...
                        if(updateWorkHosts){setUpdateWorkHosts([])} //because useEffect gets called twice
                        let tempHosts=data.hosts.map((item)=>{
                            for(let k=0; k<tempOptions.length;k++){
                                if(item==tempOptions[k].value){
                                    setUpdateWorkHosts(oldArray=>[...oldArray, tempOptions[k]])
        ...
        <div className="single-workshop-container">
            {editMode===true ? (
                <div>
                    ...
                    <div><h3>Workshop Date</h3>
                        <input type="date" id="Date" onChange={(e)=>{setUpdateDate(e.target.value)}} value={updateDate} />
                        </div>
                    <div>
                        <h3>
                            <label>
                    ...
                    onChange={(e) => setUpdateWorkStatus(e.target.value)}
                ...
                <Tabs defaultIndex={1}>
                    ...
                    <TabPanel className="tab-panel">
                        <div>
                            ...
                            <h3> {workshop.hosts&&workshop.hosts.length>0 ? <>Hosts: {workshop.hosts.join(", ")}</> : <></>}</h3>
