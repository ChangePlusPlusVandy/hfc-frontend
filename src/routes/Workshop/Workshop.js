...
export const WorkshopForm = () => {
    ...
    const makeWorkshop = (event) => {
        ...
        if (_title && _description && _numAttendees >= 0) {
            ...
            try {
                ...
                fetch("http://localhost:3000/workshop", requestOptions).then(
                    (response) => {
                        response.json().then((data) => {
                            if (data.title == _title) {
    ...
    return (
        <div className="form-container">
            ...
            <div>{message}</div>
            <div className="home">
                <button className="button" onClick={makeWorkshop}>
                    Create
                </button>
            </div>
        </div>
...
export const GetWorkshops = () => {
    let [x, setX] = useState({});
    useEffect(() => {
        findWorkshops().then((result) => setX(result));
    }, []);
